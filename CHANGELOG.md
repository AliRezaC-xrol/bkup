...
### Fixed
- **Restore reported success without actually restoring (critical)** — the restore engine only understood raw panel database files, while bkup itself produces richer formats. Restoring them "succeeded" but left the target panel untouched or even corrupted:
  - **3x-ui JSON config exports** (produced automatically whenever the database file could not be fetched) were copied verbatim onto `/etc/x-ui/x-ui.db` — a JSON text file written as the SQLite database, breaking the panel and its Xray core after restore. The engine now parses the export and **replays every inbound (and its client-traffic rows) into the panel's real schema** — discovered live via `PRAGMA` — while the fresh install's admin login, panel settings and environment are kept, so certificates and ports never break
  - **PasarGuard full snapshots** (`pasarguard_full_*.tar.gz` — manifest + JSON sections) contain no database file, so the old code just dumped the JSON files into the data directory and claimed success. The engine now maps every section (users, hosts, nodes, cores, groups, client templates) onto the panel's real SQLite tables and replays them in one transaction, then **verifies row counts table-by-table before reporting success**
  - **Rebecca's own export** (`.rbbackup`) was copied verbatim over `db.sqlite3` when the `rebecca` CLI was missing — a corrupted database. Restores that need the CLI now fail honestly with clear instructions, and archive/zip exports containing a real database are detected by content (magic bytes), not by file extension
  - **HMPanel archives** were only searched for the SQL dump at the top level — a nested official archive silently skipped the database and reported success. The whole archive tree is now searched recursively
  - every backup file is now dispatched **by content (magic bytes: SQLite / gzip / zip / JSON)**, not by file name, so reassembled or renamed backups restore correctly
  - removed every false-success path: a failed copy with a stale old database, "panel is running" with nothing restored, and unverifiable formats now produce **honest failures** with the exact reason and nothing is modified on the target server
  - same-machine (localhost) restores no longer corrupt the temporary SQL file (local and remote temp names are now distinct)
